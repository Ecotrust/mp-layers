...
const AdminLayerForm = (() => {
  const show_layertype_form = function (layertype) {
  ...
  const show_section = function (section) {
    section.removeClass('collapsed');
    section.children('h2').children('a').text('Hide');
  }

  const hide_section = function (section) {
    section.addClass('collapsed');
    section.children('h2').children('a').text('Show');
  }

  const get_wms_capabilities = function () {
  ...
  const check_queryable = function (queryable_layers) {
  ...
  const change_layer_url = function (self) {
    var url = $(this).val();
    var type = null;
   ...
   */
  const replace_all_select2_with_input = function () {
   ...
   * ALSO USED IN GeoPOrtal2.js
   */
  const replace_input_with_select2 = function (id, options) {
    var input_field = $('#' + id);
   ...
   */
  const get_catalog_records = function () {
  ...
  const show_spinner = function () {
    $('#spinner-dialog').dialog('open');
  }

  const hide_spinner = function () {
    $('#spinner-dialog').dialog('close');
  }

  const select_catalog_record = function (event, ui) {
    show_spinner();
  ...
  const enforce_organization_show = function () {
    var organization_section = $('.field-order.field-themes').parent();
    var metadata_section = $('.field-description').parent();
  ...
  /**
   * Also duplicated in layer_admin.js
   */
  const assign_field_values_from_source_technology = function () {
  ...
  return {
    ...
    populate_layer_fields_from_catalog_record,
    get_catalog_records,
    get_wms_capabilities,
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    AdminLayerForm.enforce_organization_show();

    AdminLayerForm.show_layertype_form($('#id_layer_type option:selected').text());

    console.log("CATALOG_TECHNOLOGY: '" + CATALOG_TECHNOLOGY + "'");

    AdminLayerForm.initializeCatalogIdField();

    $('#id_layer_type').change(function() {
      AdminLayerForm.show_layertype_form($('#id_layer_type option:selected').text());
      if (AdminLayerForm.assign_field_values_from_source_technology && typeof AdminLayerForm.assign_field_values_from_source_technology === "function") {
        AdminLayerForm.assign_field_values_from_source_technology();
      }
    });

    $('#id_url').blur(function() {
      AdminLayerForm.get_wms_capabilities();
    });
